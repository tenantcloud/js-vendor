		...
		function(mentioUtil, $document, $compile, $log, $timeout) {
			return {
					...
					function($scope, $timeout, $attrs) {
						...
						$scope.setTriggerText = function(text) {
							if ($scope.syncTriggerText) {
								$scope.typedTerm =
									$scope.trimTerm === undefined || $scope.trimTerm ? text.trim() : text;
							}
						};
						...
						$scope.replaceText = function(text, hasTrailingSpace) {
							...
							mentioUtil.replaceTriggerText(
								$scope.context(),
								$scope.targetElement,
								$scope.targetElementPath,
								$scope.targetElementSelectedOffset,
						...
						$scope.isContentEditable = function() {
							return (
								$scope.targetElement.nodeName !== 'INPUT' &&
								$scope.targetElement.nodeName !== 'TEXTAREA'
							);
						};
						...
						$scope.$on('menuCreated', function(event, data) {
							if ($attrs.id !== undefined || $attrs.mentioId !== undefined) {
								if (
									$attrs.id === data.targetElement ||
									($attrs.mentioId !== undefined && $scope.altId === data.targetElement)
								) {
				...
				link: function(scope, element, attrs) {
					...
					scope.$watch('iframeElement', function(newValue) {
						if (newValue) {
					...
					scope.$watch('ngModel', function(newValue) {
						...
						if (scope.triggerCharSet === undefined) {
							$log.error(
								'Error, no mentio-items attribute was provided, ' +
						...
						var mentionInfo = mentioUtil.getTriggerInfo(
							scope.context(),
							scope.triggerCharSet,
							scope.requireLeadingSpace,
							isActive
						...
						if (
							...
							(!isActive ||
								(isActive &&
									...
									((isContentEditable &&
										mentionInfo.mentionTriggerChar === scope.currentMentionTriggerChar) ||
										(!isContentEditable &&
											mentionInfo.mentionPosition === scope.currentMentionPosition))))
						) {
...
angular.module('mentio').factory('mentioUtil', [
	'$window',
	'$location',
	'$timeout',
	function($window, $location, $timeout) {
			...
			if (effectiveRange !== undefined && effectiveRange !== null) {
				...
				if (
					...
					(mostRecentTriggerCharPos === 0 ||
						!requireLeadingSpace ||
						/[\xA0\s]/g.test(
							effectiveRange.substring(mostRecentTriggerCharPos - 1, mostRecentTriggerCharPos)
						))
				) {
					var currentTriggerSnippet = effectiveRange.substring(
		...
		function getTextAreaOrInputUnderlinePosition(ctx, element, position) {
			...
			var coordinates = {
				bottom: span.offsetTop + parseInt(computed.borderTopWidth) + parseInt(computed.fontSize),
