	...
	function SwiperContainer() {
		return {
			...
			controller: [
				...
				function($scope, $element, $timeout) {
					...
					var params = {
						...
						breakpoints: $scope.breakpoints || false,
					...
					if ($scope.paginationIsActive === true) {
						params = angular.extend({}, params, {
							pagination: {
								el: '#paginator-' + $scope.swiper_uuid,
								clickable: true,
							},
						});
					}
			...
			link: function(scope, element) {
				...
				angular.element(containerElement.querySelector('.swiper-pagination')).attr('id', paginatorId);

				angular.element(containerElement.querySelector('.swiper-button-next')).attr('id', nextButtonId);

				angular.element(containerElement.querySelector('.swiper-button-prev')).attr('id', prevButtonId);

				angular.element(element[0].querySelector('.swiper-scrollbar')).attr('id', scrollBarId);
			},

			template:
				'<div class="swiper-container {{containerCls}}">' +
				...
				'</div>' +
				'<div class="swiper-button-next" ng-show="showNavButtons" data-selenium="next-btn"></div>' +
