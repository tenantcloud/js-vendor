...
(function ($) {
	...
	var maskBind = function () {
		...
		//fix: Android doesn't support keypress event. Use input event instead
		if(isAndroid) {
			$(this).on("input.inputmasks", masksInput);
		} else {
			$(this).on("keypress.inputmasks", masksKeyPress);
		}
		...
		$(this)
			...
			.on("setvalue.inputmasks", masksChange) //masksSet
	...
	var masksInput = function (e) {
		...
		keyboardApply.call(this, e, text, true);

		// Fix insert number value after country scheme change
		$(e.target).inputmask("setvalue", text);
	};
