...
(function($) {
	...
	Format.prototype.to = function(number) {
		...
		return this.v('to')(preNegative + this.v('prefix') + negative + base + mark + this.v('postfix'));
	};
	...
	function Link(entry, update) {
		...
		return new Link.prototype.init(
	...
	Link.prototype.setField = function(target) {
		...
		this.target = target.on('change', function(e) {
			that.obj.val(at(null, $(e.target).val(), that.N), { link: that, set: true });
		});
	};
	...
	Link.prototype.setFormatting = function(options) {
		this.formatting = new Format(
			$.extend({}, options, this.formatting instanceof Format ? this.formatting.settings : this.formatting)
		);
	};
