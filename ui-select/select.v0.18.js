	...
	uis.controller('uiSelectCtrl', [
		...
		function(
			...
			ctrl.select = function(item, skipFocusser, $event) {
				// n.kovtun: fix undefined
				if (item === null || !_isItemDisabled(item)) {
		...
		function($document, uiSelectConfig, uiSelectMinErr, uisOffset, $compile, $parse, $timeout) {
			return {
				...
				templateUrl: function(tElement, tAttrs) {
					...
					return (
						theme + (angular.isDefined(tAttrs.multiple) ? '/select-multiple.tpl.html' : '/select.tpl.html')
					);
				},
				replace: true,
				transclude: true,
			...
			return {
				...
				controller: [
					...
					function($scope, $timeout) {
						...
						ctrl.getPlaceholder = function() {
							//Refactor single?
							// fix Nazar K.
							if ($select.items && $select.items.length === 0) return;
				...
				link: function(scope, element, attrs, ctrls) {
					...
					ngModel.$formatters.unshift(function(inputValue) {
						var data =
								$select.parserResult && $select.parserResult.source(scope, { $select: { search: '' } }), //Overwrite $search
							locals = {},
							result;
			...
			return {
				...
				link: function(scope, element, attrs, ctrls) {
					...
					ngModel.$parsers.unshift(function(inputValue) {
						...
						// n.kovtun: fix return result;
						return _.isNull(inputValue) ? null : result;
					});
					...
					ngModel.$formatters.unshift(function(inputValue) {
						...
						// n.kovtun: fix return inputValue;
						return _.isNull(inputValue) ? null : inputValue;
					});
					...
					// prevent keyboard from showing after closing ui-select
					var focusser = angular.element(
						"<input ng-disabled='$select.disabled' class='ui-select-focusser ui-select-offscreen' type='text' id='{{ $select.focusserId }}' aria-label='{{ $select.focusserTitle }}' aria-haspopup='true' role='button' readonly />"
					);
